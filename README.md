...
vcf-ocp-migration automates moving an OpenShift cluster from a source vCenter to a target vCenter. The project consists of:
...
```sh
make operator-image operator-push IMG=<some-registry>/vcf-ocp-migration:tag
```
...
```sh
make deploy IMG=<some-registry>/vcf-ocp-migration:tag
```
...
```sh
make build-installer IMG=<some-registry>/vcf-ocp-migration:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/vcf-ocp-migration/<tag or branch>/dist/install.yaml
```
